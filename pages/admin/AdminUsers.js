...
var AdminUsers = function () {
        ...
        return __generator(this, function (_a) {
            switch (_a.label) {
                ...
                case 1:
                    ...
                    return [4 /*yield*/, adminApi_1.adminApi.getUsers(filters)];
                case 2:
                    data = _a.sent();
                    setUsers(data.users);
    ...
    return (<div className="space-y-6">
            ...
            <div className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex gap-4 flex-wrap">
                <div className="flex-1 min-w-[200px]">
                    ...
                    <div className="relative">
                        <input type="text" placeholder="İsim veya email..." className="w-full pl-10 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" value={filters.searchQuery} onChange={function (e) { return setFilters(__assign(__assign({}, filters), { searchQuery: e.target.value, page: 1 })); }}/>
                        <lucide_react_1.Search className="w-5 h-5 text-gray-400 absolute left-3 top-2.5"/>
                    </div>
                </div>

                <div className="w-48">
                    <label className="block text-sm font-medium text-gray-700 mb-1">Rol</label>
                    <select className="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" value={filters.role} onChange={function (e) { return setFilters(__assign(__assign({}, filters), { role: e.target.value, page: 1 })); }}>
                ...
                <div className="w-48">
                    ...
                    <select className="w-full border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" value={filters.accountStatus} onChange={function (e) { return setFilters(__assign(__assign({}, filters), { accountStatus: e.target.value, page: 1 })); }}>
                        <option value="">Tümü</option>
            ...
            <div className="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {loading ? (<tr>
                                    ...
                                    <td colSpan={5} className="px-6 py-4 text-center text-gray-500">Kullanıcı bulunamadı</td>
                                </tr>) : (users.map(function (user) { return (<tr key={user.uid} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={"px-2 inline-flex text-xs leading-5 font-semibold rounded-full ".concat(getRoleBadge(user.role))}>
                                                {user.role}
                                            </span>
                                        </td>
                                        <td className="px-6 py-4">
                                            {user.accountStatus === 'banned' ? (<span className="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-red-100 text-red-800">
                                                    Banlı
                ...
                if (!user.createdAt)
                    return '-';
                var seconds = user.createdAt.seconds || user.createdAt._seconds;
                if (seconds)
                ...
                return new Date(user.createdAt).toLocaleDateString('tr-TR');
                                        ...
                                        <td className="px-6 py-4 text-right text-sm font-medium">
                                            <button onClick={function () { return navigate("/admin/users/".concat(user.uid)); }} className="text-indigo-600 hover:text-indigo-900">
